"""
Test script for DDPG algorithm on SIM-based beamforming.
Follows the same flow as main.py but uses Deep Deterministic Policy Gradient.
"""
import torch
import numpy as np
...
from simpy.algorithm import DDPG
...
print("\n" + "="*80)
print("DDPG TESTING FOR SIM-BASED BEAMFORMING")
print("="*80)
...
# ========== Setup DDPG ==========
print("\n" + "="*80)
print("CONFIGURING DDPG ALGORITHM")
print("="*80)
...
print(f"   DDPG agent created")
print(f"   Actor network: {state_dim} -> 256 -> 256 -> {action_dim}")
print(f"   Critic network: ({state_dim} + {action_dim}) -> 256 -> 256 -> 1")

# ========== Run DDPG Optimization Multiple Times ==========
print("\n" + "="*80)
print("RUNNING DDPG OPTIMIZATION (50 RUNS)")
print("="*80)
...
for i in range(num_runs):
    ...
    # Reset DDPG agent for each run (fresh networks)
    ddpg_agent = DDPG(
        beamformer=sim_beamformer,
...
plt.subplot(1, 2, 1)
plt.scatter(range(num_runs), sumrate, alpha=0.6, s=50)
plt.axhline(y=np.mean(sumrate), color='r', linestyle='--',
            label=f'Mean: {np.mean(sumrate):.4f}')
plt.xlabel('Run Number')
plt.ylabel('Sum-Rate (bits/s/Hz)')
plt.title(f'DDPG: Sum-Rate across {num_runs} Runs')
plt.grid(True, alpha=0.3)
plt.legend()

# Plot 2: Histogram
plt.subplot(1, 2, 2)
plt.hist(sumrate, bins=20, alpha=0.7, edgecolor='black')
...
print("\n   Results plot saved to 'ddpg_results.png'")
