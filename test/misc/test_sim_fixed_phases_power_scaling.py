"""
Test SIM beamforming with FIXED optimized phases across different power levels.

This isolates whether the issue is:
1. Poor phase optimization at each power level, OR
2. Fundamental channel conditioning issues

Strategy:
- Optimize phases ONCE at a reference power level using PGA
- Use those SAME phases for all other power levels
- Check if SINR scales properly with power
"""
...
sim_layers = 2
sim_metaatoms = 25
sim_layer_spacing = 0.1 #wavelength / 4
sim_metaatom_spacing = 0.05# wavelength / 2
sim_metaatom_area = 0.01 * 0.01  # Paper: 0.0001 m² (NOT 0.0025 m²)
...
print("\n" + "="*80)
print("SIM FIXED PHASES POWER SCALING TEST")
print("="*80)
print(f"Strategy: Optimize phases at {reference_power_db} dBm, then test across all powers")
...
# print(f"\n3. Optimizing phases at reference power ({reference_power_db} dBm)...")
...
# Run PGA with LONG optimization (5000 iterations for better interference cancellation)
optimizer = PGA(
    ...
    verbose=False,  # Disable verbose for long run
)
...
# print(f"\n   Optimized sum-rate at {reference_power_db} dBm: {sumrate_at_ref:.4f} bits/s/Hz")

# Compute effective channel with optimized phases
...
print("\n" + "="*80)
print("TESTING FIXED OPTIMIZED PHASES ACROSS POWER LEVELS")
print("="*80)
...
if capacity_increase_equal > power_increase_db * 0.1:
    print(f"\n✓ Capacity IS increasing with power (though may be suboptimal)")
else:
    print(f"\n✗ Capacity NOT increasing properly - fundamental channel issue")
...
ax.set_title('SIM (Fixed Optimized Phases): Capacity vs Power', fontsize=14, fontweight='bold')
...
for user_idx in range(num_users):
    ...
    power_alloc_dbm = np.where(power_alloc_array[:, user_idx] > 0,
                                ...
                                -np.inf)
    power_alloc_dbm = np.array(power_alloc_dbm)
    ax.plot(results['power_db'], power_alloc_dbm+1.5,
            's-', label=f'User {user_idx}', linewidth=2, markersize=6)
ax.set_xlabel('Transmit Power (dBm)', fontsize=12)
...
plt.tight_layout()
plt.savefig('test/test_sim_fixed_phases_power_scaling.png', dpi=150, bbox_inches='tight')
print("✓ Plot saved to: test/test_sim_fixed_phases_power_scaling.png")

print(power_alloc_array)
